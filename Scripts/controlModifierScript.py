...
def createControl(originalJnt, colour, controlSize, controlType):
    jntControl = createControlType(originalJnt+"Control", controlSize, controlType)
...
def createControlType(controlName, controlSize, controlType):
    if controlType == 1:
        newCtrl = createNurbsCube(controlName, controlSize)
    elif controlType == 2:        
        newCtrl = createNurbsArrow(controlName, controlSize)
    elif controlType == 3:
        newCtrl = cmds.circle(name=controlName, radius=controlSize, nr=(1, 0, 0))[0]    
    return newCtrl
...
def createSingleControl(radiusField, controlColour, newControlRadio):
    #'Cube', 'Arrow', 'Circle'
    ...
    if not selected:
        ...
        jntControl, jntControlGrp = createControl(jnt, controlColour, controlSize, controlType)
...
def swapControlShape(radiusField, controlColour, newControlRadio):
    #'Cube', 'Arrow', 'Circle'
    ...
    for con in controls:   
        newCtrl = createControlType("tempSwapCtrl", controlSize, swapControlResults)     
...
def createWindow():
    ...
    cmds.button(
        ...
        command=lambda *args: createSingleControl(radiusField, savedCol[0], newControlRadio)
    )   
    ...
    cmds.text(label='X:', align='right', annotation="The size of the controls for the joints.")
    scaleXField = cmds.intField(width=40, value = 1, min=0.1)
    cmds.text(label='Y:', align='right', annotation="The size of the controls for the joints.")
    scaleYField = cmds.intField(width=40, value = 1, min=0.1)
    cmds.text(label='Z:', align='right', annotation="The size of the controls for the joints.")
    scaleZField = cmds.intField(width=40, value = 1, min=0.1)
    scaleFields = {
        "x":scaleXField,
        "y":scaleYField,
        "z":scaleZField
    ...
    fk_to_ik_button = cmds.button(label='Transform Shape', parent=mainLayout, annotation="Select created the switch: Match the pose between IK and FK.",
                command=lambda *args: transformShape(translateFields, rotationFields, scaleFields))
    ...
    fk_to_ik_button = cmds.button(label='Change Shape', parent=mainLayout, annotation="Select created the switch: Match the pose between IK and FK.",
                command=lambda *args: swapControlShape(radiusField, savedCol[0], newControlRadio))
...
def transformShape(translateFields, rotationFields, scaleFields):
    translateValues = {
    ...
    for con in controls:
        ...
        for shape in oldShapes:
            ...
            scaleShape(shape,scaleValues["x"],scaleValues["y"],scaleValues["z"])            
    
def rotateShape(shapeNode, x=0, y=0, z=0):
    cmds.rotate(
        x, y, z,
        shapeNode + ".cv[*]",
        relative=True,
...
def moveShape(shapeNode, x=0, y=0, z=0):
    cmds.move(
        x, y, z,
        shapeNode + ".cv[*]",
        relative=True,
...
def scaleShape(shapeNode, x=0, y=0, z=0):
    cmds.scale(
        x, y, z,
        shapeNode + ".cv[*]",
        relative=True,
